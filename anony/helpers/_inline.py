...
class Inline:
    ...
    def stream_markup(self, _lang: dict, chat_id: int, status: bool, stype: str = "audio", source: str = "url", loop: bool = False, notify: bool = True) -> types.InlineKeyboardMarkup:
        builder = InlineKeyboardBuilder()
        builder.button(text=_lang["stream_status"] + (": ON" if status else ": OFF"), callback_data="none")
        builder.button(text=_lang["stream_notify"] + (": ON" if notify else ": OFF"), callback_data=f"toggle_notify {chat_id}")
